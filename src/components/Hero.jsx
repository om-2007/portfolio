...
export default function Hero() {
    ...
    const handleDownloadResume = useCallback(async () => {
        ...
        try {
            // Wait a tick for the hidden div to mount, then download
        ...
        <section id="hero" ref={revealRef}>
            <div className="hero-content">
                ...
                <div className="hero-cta">
                    ...
                    <button onClick={handleDownloadResume} disabled={generatingPDF} className="btn secondary" style={{ background: 'none', border: '1px solid var(--border)', color: 'var(--text-h)', cursor: 'pointer' }}>
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ marginRight: 8 }}>
                            <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
                            <polyline points="7 10 12 15 17 10" />
                            <line x1="12" y1="15" x2="12" y2="3" />
                        </svg>
                        {generatingPDF ? 'Generating…' : 'Resume'}
                    </button>
                </div>
            </div>
            {/* Hidden resume container for PDF generation — offscreen but NOT fixed, so html2canvas captures correctly */}
            <div id="resume-pdf-container" style={{ position: 'absolute', left: '-9999px', top: 0, width: '816px', background: '#fff', color: '#1a1a2e', fontFamily: 'system-ui, -apple-system, sans-serif', fontSize: '12.5px', lineHeight: '1.5', overflow: 'hidden' }}>
